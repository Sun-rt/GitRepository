...
function display_pollvote($poll_id, $display_loading = true) {
	...
	//问题css
	$template_question = str_replace("style=\"text-align: center;\"", "style=\"height: 40px; margin-top: 10px; color: #B14213; font-size: 18px\"", $template_question);
	...
	if($poll_question && $poll_answers) {
		...
		foreach($poll_answers as $poll_answer) {
			...
			//选项css
            $template_answer = str_replace("<li>", "<h4 style=\"height: 25px;font-size: 17px;\">", $template_answer);
            $template_answer = str_replace("</li>", "</h4>", $template_answer);
...
function vote_poll() {
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'polls')
	{
		...
		foreach ($polls_set as $poll) {
			// Load Headers
		polls_textdomain();
		header('Content-Type: text/html; charset='.get_option('blog_charset').'');

		// Get Poll ID
		$poll_id = $poll["poll_id"];//(isset($_REQUEST['poll_id']) ? intval($_REQUEST['poll_id']) : 0);
		var_dump($poll_id);
		...
		switch($view )
		{
			// Poll Vote
			case 'process':
				$poll_aid = $poll["poll_ans_id"];;//$_POST["poll_$poll_id"];
